...
MotorController::MotorController() : isMoving(false), lastStatusTime(0) {
    motorNames[0] = 'X'; motorNames[1] = 'Y'; motorNames[2] = 'Z'; 
    motorNames[3] = 'T'; motorNames[4] = 'G';
}
...
void MotorController::loop() {
    ...
    // Periodic status during movement
...
void MotorController::initializeMotors() {
    motors[0] = new AccelStepper(AccelStepper::DRIVER, 2, 3);   // X
    motors[1] = new AccelStepper(AccelStepper::DRIVER, 4, 5);   // Y
    motors[2] = new AccelStepper(AccelStepper::DRIVER, 6, 7);   // Z
    motors[3] = new AccelStepper(AccelStepper::DRIVER, 8, 9);   // T
    motors[4] = new AccelStepper(AccelStepper::DRIVER, 10, 11); // G
...
void MotorController::parseGroupMove(const String& cmd) {
    // Simple group move - all motors finish together
    parseMove(cmd); // For now, same as regular move
}
