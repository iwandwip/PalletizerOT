...
void StatusManager::updatePosition(long X, long Y, long Z, long T, long G) {
  ...
  if (newPosition.X != currentPosition.X || 
      ...
      newPosition.T != currentPosition.T || 
      newPosition.G != currentPosition.G) {
...
String StatusManager::generateStatusReport() const {
  return "STATUS:" + getSystemStatusString() + 
         " QUEUE:" + String(metrics.queueLength) +
         " CMD:" + String(metrics.commandsProcessed) +
         " ERR:" + String(metrics.errorsCount) +
         " HEAP:" + String(metrics.freeHeap);
}

String StatusManager::generatePositionReport() const {
  return "P X" + String(currentPosition.X) + 
         " Y" + String(currentPosition.Y) + 
         " Z" + String(currentPosition.Z) + 
         " T" + String(currentPosition.T) + 
         " G" + String(currentPosition.G);
}

String StatusManager::generateMetricsReport() const {
  return "METRICS UPTIME:" + String(metrics.uptime) +
         " HEAP:" + String(metrics.freeHeap) +
         " CMD:" + String(metrics.commandsProcessed) +
         " ERR:" + String(metrics.errorsCount) +
         " RESP:" + String(metrics.averageResponseTime, 2) + "ms";
}
...
void StatusManager::updateSystemHealth() {
  ...
  // Check heap memory
  if (metrics.freeHeap < 10000) { // Less than 10KB free
    systemHealthy = false;
  }
...
void StatusManager::parsePositionString(const String& positionString) {
  // Parse position string format: "P X1000 Y2000 Z0 T0 G0"
  ...
  // Simple parsing - look for axis letters followed by numbers
